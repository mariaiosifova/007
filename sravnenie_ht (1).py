...
def plot_mass_vs_time(filename):
    ...
    mass = [item['altitude'] for item in data]

    # Построение графика
    plt.figure(figsize=(10, 6))
    plt.plot(time, mass, 'b-', linewidth=2, label='Данные из полета в KSP')
    plt.plot(ht.time, ht.heights, 'r--', linewidth=2, label='Математическая модель', alpha=0.7)
    plt.title('Зависимость высоты от времени', fontsize=14)
